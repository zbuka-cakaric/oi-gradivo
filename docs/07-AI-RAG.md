...
## CHANGELOG
- 2.11 (2026-07-06): **v071 BUGFIX Vještak TDZ**. anthropicStreamAlati: kolizija imena `misli` (boolean thinking-flag na razini funkcije + objekt thinking-blokova u try-bloku) → runtime "Cannot access 'misli' before initialization" kad je thinking aktivan. Objekt preimenovan u misliMap. Latentno od v065, aktiviralo se korištenjem thinking grane. Pouka: block-scoped const kolizije NE hvata node --check (samo runtime).
- 2.10 (2026-07-06): **v070 BUGFIX temperature**. modelBezTemp(m) prošireno: uz sonnet-5|fable|mythos sada hvata i opus-X-Y kad je maj>4 ili maj==4&min>=7 (Opus 4.7/4.8/5+ deprecirali temperature → 400). Svi pozivi (anthropicPoziv, anthropicStream, anthropicStreamAlati, thinking grana) već rutaju kroz modelBezTemp pa je fix centralan. Uzrok: usmeni koristi MODEL_ODGOVOR (preporučeno Opus za Vještaka/kvalitetu) → slao temperature → 400.
- 2.9 (2026-07-06): **v069 — komisija + vještak dubina**. KOMISIJA (server): const KOMISIJA[] 4 člana s područjima (zaštita na radu/zaštita od požara/organizacija građenja/opće), odaberiClana(tekstPitanja) boduje po ključnim riječima → član (OTR/Horvat proširen s gradnja/građenje/rekonstrukcija/norma/proizvod da opći/Novak bude pravi fallback; raspodjela na banci 296: otr 45%, opći 30%, znr 15%, zop 9% — realno stručno). P5_ISPITIVAC dobiva "Tvoja uloga: ime, član komisije za područje" pa govori u karakteru. Vraća se clan{ime,inicijal,podrucje} u start+odgovori (🔒 bez zlatnog sadržaja). VJEŠTAK: vjestakPetlja šalje status "🤔 analiziram…" kad AI_THINKING=1 (Opus dubina vidljiva korisniku). Trošak komisije = 0 (deterministički keyword izbor, bez AI poziva).
- 2.8 (2026-07-05): **v065 — Ivanov feedback nakon testiranja usmenog**. (1) Usmeni ocjenjuje po SMISLU, ne doslovnosti: P6_PROCJENA + P7A_ISPRAVAK dobili pravilo da nagrađuju razumijevanje i slobodnu interpretaciju članaka ako se misli na istu stvar; ne snižavaju zbog izostanka broja članka/sinonima. (2) Prag prolaza 90→80 (USMENI_PROLAZ + TEST_PROLAZ default 80; ENV i dalje nadjačava; graf napretka linija na 20%). (3) Vještak extended thinking: `anthropicStreamAlati` prima thinking preko ENV **AI_THINKING=1** (budžet AI_THINKING_BUDZET def 2000); hvata thinking/redacted_thinking blokove u streamu (NE šalju se korisniku), vraća ih u misliBlok i stavlja na POČETAK assistant poruke pri tool-use (API zahtjev). Preporuka: uz Opus (MODEL_ODGOVOR=claude-opus-4-8) za Vještaka — Pro-feature s kvotom, kvaliteta rasuđivanja opravdava trošak. Vještak već ima živi stream+karet (nema nove UI potrebe). UI 61/61.
- 2.7 (2026-07-05): **v064 — usmeni DISTRAKTOR-ZAMKE**. Kad pitanje ima abc opcije, `usmeniSesijaVlasnika` povlači i `opcije`/`tip`; P6 podloga dobiva netočne distraktore kao "MOGUĆE ZAMKE"; P6 prompt: kad kandidat solidno odgovori, SMIJE povremeno (ne uvijek) umjesto dopune ponuditi netočnu tvrdnju kao "a je li točno da…?" — kao prava komisija; nikad kao hint kad ne zna. Aktivira se SAMO za tip='abc' s opcijama (otvorena/usmena pitanja netaknuta — bez regresije). Distraktori dolaze iz abc-konverzije banke (isti izvor, dvostruka korist).
